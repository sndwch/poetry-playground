...
class PoemGenerator:
    ...
    def poem_line_from_markov(
        self,
        starting_word: str,
        num_words: int = 4,
        rhyme_with: Optional[str] = None,
        words_for_sampling: List[str] = [],
        max_line_length: Optional[int] = 35,
    ) -> str:
        ...
        for i in range(num_words - 1):
            if (i == num_words - 2) or (
                max_line_length
                and (max_line_length > 14 and len(" ".join(output_words)) >= max_line_length - 14)
            ):
                ...
                word = markovgen.last_word_of_markov_line(
                    output_words, rhyme_with=rhyme_with, max_length=max_word_length
                )
                output_words.append(word)
                break
            else:
                word = markovgen.nonlast_word_of_markov_line(
                    output_words, words_for_sampling=words_for_sampling
                )
                output_words.append(word)
    ...
    def poem_from_markov(
        ...
        """
        self.poem = None
        words_for_sampling = input_words + phonetically_related_words(
            input_words, limit_results_per_input_word=20
        )
        ...
        for word_pair in similarity_checks:
            if not (word_pair[0] in words_removed or word_pair[1] in words_removed) and too_similar(
        ...
        for i in range(num_lines):
            ...
            line_starter = (
                words_for_sampling.pop()
                if random.random() > 0.4
                else random.choice(StochasticJolasticWordGenerator.common_words)
            )
            ...
            line = self.poem_line_from_markov(
                line_starter,
                words_for_sampling=words_for_sampling,
    ...
    def poem_line_from_word_list(
        self, word_list: List[str], max_line_length=35, connectors: List[str] = []
    ) -> str:
        ...
        for word in word_list[1:]:
            if random.random() < (
                0.2 + len(output) / 100
            ):  # Increasing probability of line termination as line gets longer
                break
    ...
    def poem_from_word_list(
        ...
        connectors: List[str] = [],
        limit_line_to_one_input_word: bool = False,
    ):
        ...
        if limit_line_to_one_input_word:
            for i in range(num_lines - 1):
                ...
                output += self.poem_line_from_word_list(
                    phonetically_related_words(linked_word),
                    connectors=connectors,
                    max_line_length=max_line_length,
                )
                line_indent = (
                    random.choice(self.line_indents)
                    if line_indent == ""
                    else random.choice([li for li in self.line_indents if li is not line_indent])
            ...
            for i in range(num_lines - 1):
                ...
                output += self.poem_line_from_word_list(
                    word_list, connectors=connectors, max_line_length=max_line_length
                )
                # Don't repeat the same indent 2x
                line_indent = (
                    random.choice(self.line_indents)
