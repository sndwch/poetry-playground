...
class TestUtils(unittest.TestCase):
    ...
    def test_get_random_color(self):
        for i in range(4):
            ...
            red_over = rgb[0] >= 0.85
            green_over = rgb[1] >= 0.85
            blue_over = rgb[2] >= 0.85
            self.assertFalse(red_over and green_over) or (red_over and blue_over) or (
                green_over and blue_over
            )
        for i in range(4):
            threshold = 0.25
            rgb = get_random_color()
    ...
    def test_sort_by_rarity(self):
        ...
        # Order may vary for words with equal rarity, so check the actual output
        correctly_sorted_words = ["catabasis", "corncob", "errant", "Catalan", "hue", "cat", "the"]
...
class TestLexigen(unittest.TestCase):
    ...
    def test_similar_meaning_word(self):
        ...
        try:
                ...
                results = contextually_linked_words(
                    "metamorphosis", sample_size=None, datamuse_api_max=None
                )
                self.assertGreater(len(results), 20)
                ...
                contextually_linked_to_crepuscular = [
                    "foraging",
                    "dusk",
                    "habits",
                    "twilight",
                ...
                contextually_linked_to_either = sorted(
            ...
            try:
                    ...
                    self.assertIn(
                        contextually_linked_word("metamorphosis"), contextually_linked_to_metamorphosis
                    )

                def test_frequently_following_words(self):
        try:
                        self.assertEqual(frequently_following_words("nonexistentword"), [])
                        frequently_following_magic = [
                            "about",
                            "against",
                            "among",
                            "and",
                        ...
                        ff_either = sorted(
                            ...
                            + [
                                word
                                for word in frequently_following_Dadaist
        ...
        try:
                                    ...
                                    self.assertEqual(
                                        sorted(related_rare_words(["comical", "dinosaur"], sample_size=None)), rr_to_either
                                    )
            ...
            try:
                                ...
                                class TestPoemGenerator(unittest.TestCase):
                                    ...
                                    def test_poem_line_from_word_list(self):
                                        ...
                                        for i in range(5):  # Generate 5 random lines of poetry and test them.
                                            ...
                                            poem_line = pgen.poem_line_from_word_list(
                                                input_word_list, max_line_length=max_line_length
                                            )
                                            ...
                                            self.assertTrue(
                                                set(possible_words + possible_connectors).issuperset(set(poem_line.split()))
                                            )
                                ...
                                class TestPDFPNGGenerator(unittest.TestCase):
                                    def test_get_font_sizes(self):
                                        pdfgen = PDFGenerator()
                                        font_size = pdfgen.get_font_size(
                                            "abra cadabra hocus pocus bananas eating locusts"
                                        )  # 47 characters
                                    ...
                                    def test_get_max_x_coordinate(self):
                                        ...
                                        x_coordinate = pdfgen.get_max_x_coordinate(
                                            "this line is thirty-five characters", "Arial", 21
                                        )
                                        self.assertEqual(x_coordinate, 60)
                                    ...
                                    def test_set_filename(self):
                                        ...
                                        input_words = ["chalice", "crime", "coins", "spectacular", "dazzle", "enigma"]
                                        self.assertEqual(
                                            pdfgen.set_filename(input_words), "chalice,crime,coins,spectacular,dazzle,enigma.pdf"
                                        )
                                        self.assertEqual(
                                            pdfgen.set_filename(input_words, file_extension="png"),
                                ...
                                class TextExtractionTestCase(unittest.TestCase):
                                    def test_validate_url(self):
                                        ...
                                        self.assertRaises(
                                            Exception, lambda: validate_url("http://test.com", expected_netloc="gutenberg.org")
                                        )
                                        self.assertRaises(
                                            Exception,
                                            lambda: validate_url(
                                        ...
                                        validate_url(
                                            "https://www.gutenberg.org/files/11/11-h/11-h.htm", expected_netloc="gutenberg.org"
                                        )
                                        validate_url(
                                        ...
                                        self.assertRaises(
                                            Exception,
                                            lambda: validate_url(
                                                "https://www.gutenberg.org/ebooks/11", expected_netloc="archive.org"
                                            ),
                                        )
                                        validate_url(
                                ...
                                class TextProcessingTestCase(unittest.TestCase):
                                    def test_reconcile_replacement_word(self):
                                        ...
                                        self.assertEqual(
                                            reconcile_replacement_word(" bicycles ", "NNS", "lemurs", "NNS"), " lemurs "
                                        )
                                    ...
                                    def test_swap_parts_of_speech(self):
                                        ...
                                        for i, token in enumerate(new_sh_doc):
                                            ...
                                            if expected_pos == "ADJ":
                                                ...
                                                self.assertTrue(
                                                    (
                                                        token.text in great_expectations_nouns
                                                        or inflector.plural(token.text) in great_expectations_nouns
                                                        or inflector.singular_noun(token.text) in great_expectations_nouns
                                                    )
                                                    or i > 20
                                                )
                                    ...
                                    def test_markov(self):
                                        ...
                                        self.assertEqual(len(output_sentences), 5)
                                        output_sentences = markov(
                                            [alice_in_wonderland, cosmicomics], ngram_size=2, num_output_sentences=3
                                        )
                                        print(
                                    ...
                                    def test_cutup(self):
                                        ...
                                        for cutout in cutouts:
                                            ...
                                            self.assertTrue(
                                                3 <= len(cutout.split()) <= 7 or cutout.split()[-1] == burroughs_sample1.split()[-1]
                                            )
                                        ...
                                        for cutout in cutouts:
                                            self.assertTrue(
                                                2 <= len(cutout.split()) <= 10
                                                or cutout.split()[-1] == burroughs_sample1.split()[-1]
                                            )
                                            self.assertIn(cutout, burroughs_sample1)
                                        ...
                                        for cutout in cutouts:
                                            ...
                                            self.assertTrue(
                                                len(cutout.split()) <= 7
                                                or cutout.split()[-1] == burroughs_sample1.split()[-1]
                                                or cutout == burroughs_sample2.split()[-1]
                                            )
                                            self.assertTrue(cutout in burroughs_sample1 or cutout in burroughs_sample2)
                                        cutouts = cutup(
                                            [burroughs_sample1, burroughs_sample2], min_cutout_words=2, max_cutout_words=10
                                        )
