...
from generativepoetry.metaphor_generator import (
    Metaphor,
    MetaphorGenerator,
    MetaphorType,
)
...
def test_generate_metaphor_batch():
    ...
    for i, metaphor in enumerate(metaphors[:5], 1):
        print(f"{i}. [{metaphor.metaphor_type.value}] {metaphor.text} (score: {metaphor.quality_score:.2f})")
...
def test_new_patterns_in_batch_generation():
    ...
    print(f"Found {sum(1 for m in metaphors if m.metaphor_type == MetaphorType.CONCEPTUAL)} conceptual metaphors")
